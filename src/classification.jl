...
function quantile(fc::GeoJSON.FeatureCollection, field::AbstractString, percentiles::Vector{Float64})
    vals = Float64[feature.properties[field] for feature in fc.features]
    Float64[Base.quantile(vals, percentile * 0.01) for percentile in percentiles]
end
quantile(fc::GeoJSON.FeatureCollection, field::AbstractString, percentiles::Vector{Int}) = quantile(fc,field,float(percentiles))
...
function reclass(fc::GeoJSON.FeatureCollection, infield::AbstractString, outfield::AbstractString, translations::Vector)
